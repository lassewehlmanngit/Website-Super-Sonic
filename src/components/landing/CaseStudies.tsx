...
export const CaseStudies: React.FC<CaseStudiesProps> = ({ lang }) => {
  ...
  return (
    <section id="case-studies" className="py-20 md:py-32 bg-paper">
      <div className="container-responsive">
        <div className="text-center mb-16 reveal">
          <p className="text-sonic-orange font-mono text-sm uppercase tracking-widest mb-4">
            {isDe ? "Fallstudien" : "Case Studies"}
          </p>
          <h2 className="text-3xl md:text-5xl lg:text-6xl font-bold text-black tracking-tighter mb-4">
            {isDe 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {cases.map((caseStudy, i) => (
            <div key={i} className="bg-white rounded-3xl p-8 md:p-10 border border-zinc-100 shadow-sm hover:shadow-xl transition-all reveal delay-100">
              {/* Header */}
              ...
              <div className="mb-4">
                ...
                <p className="text-zinc-600 mt-1">{caseStudy.problem}</p>
              </div>

              {/* Solution */}
              <div className="mb-4">
                <span className="text-xs font-bold text-sonic-orange uppercase tracking-wider">
                  {isDe ? "Lösung" : "Solution"}
                </span>
                <p className="text-zinc-600 mt-1">{caseStudy.solution}</p>
              </div>
              ...
              <div className="bg-green-50 rounded-xl p-4 border border-green-100">
                ...
                <p className="text-black font-medium mt-1">{caseStudy.result}</p>
              </div>
            </div>
          ))}
        </div>
